  ...
  return (
    <AnimatedSection delay={index * 0.1}>
      ...
      >
        ...
        <span
          className={`pointer-events-none absolute bottom-6 left-0 top-6 w-px rounded-full bg-accent-cyan transition-all duration-300 ${
        ...
        <button
          type="button"
          onClick={onToggle}
          className="relative block w-full px-5 py-5 text-left active:scale-[0.995] transition-transform sm:px-6 sm:py-6"
          aria-expanded={open}
        >
          <div>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <p
                className={`${dmMono.className} text-[10px] font-medium uppercase tracking-[0.32em] text-accent-indigo sm:text-[11px]`}
              >
                {role.duration}
              </p>
              <span
                className={`${dmMono.className} inline-flex items-center gap-2 rounded-full border px-3 py-2 text-[10px] uppercase tracking-[0.28em] transition duration-300 ${
                  open
                    ? 'border-accent-cyan/[0.18] bg-accent-cyan/[0.08] text-text-bright'
              ...
              >
                ...
                <span
                  className={`inline-flex h-5 w-5 items-center justify-center rounded-full border transition duration-300 ${
                    open
                      ? 'border-accent-cyan/20 bg-accent-cyan/[0.12] text-accent-cyan'
            ...
            <div className="mt-4 min-w-0">
              ...
              <p className="mt-3 text-[0.96rem] leading-6 text-accent-cyan">
                {role.company}
              </p>
              {role.companyDetail && (
              ...
              <div className="relative mt-5">
                <p
                  className={`max-w-[46rem] overflow-hidden text-sm leading-7 text-text-secondary transition-[max-height,color] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                    open ? 'max-h-72 text-text-secondary/[0.88]' : 'max-h-[3.75rem]'
                  }`}
                >
                  {role.description}
                </p>
        ...
        <AnimatePresence initial={false}>
          {open && (
            ...
            >
              <div className="border-t border-white/[0.06] pb-6 pt-4">
                <ul className="flex flex-wrap gap-2.5">
                  {role.skills.map((skill) => (
                    <li
                      key={skill}
                      className="rounded-full border border-mint/[0.14] bg-mint/[0.05] px-3 py-1.5 text-xs font-medium text-mint"
                    >
                      {skill}
                    </li>
                  ))}
